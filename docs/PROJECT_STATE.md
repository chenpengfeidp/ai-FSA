...
- Last updated: 2026-07-30 (P2E.5 Projection Replay Validation — production code in `@fas/statistics` + `@fas/analysis`)
...
- Current sprint: **P2E.5** Projection Replay Validation complete (production code). Deterministic `ReplayRunner` in `@fas/statistics` compares Projection V1 (sealed snapshot) vs V2 (sidecar re-projection) on read-only Evaluation History; `AnalysisProjectionReplayPort` in `@fas/analysis` implements the port; grouped comparison report with A2 calibration reuse. **Governance note:** P2A–P2E.5 not yet listed in doc 40.
- Last completed delivery: Sprint **P2E.5** (`docs/sprints/P2E.5/P2E.5_PROJECTION_REPLAY_VALIDATION_COMPLETION_REPORT.md`); prior P2E, P2D, P2C, P2B, P2A, M1A, O1, V1A, A2, P1B, P1A, L1B, L1A, DA, P0, A1.5, A1, R1, I2B, I2A, I1B, I1A, F1.3B, F1.3A, F1.2b, F1.2a, F1.1E
