...
- Current task status: **L1A Club Intelligence Evidence** complete; prior **DA** Domain Architecture + **P0** + **A1.5** complete; product roadmap remains `docs/40_PRODUCT_ROADMAP.md`
- Delivery phase: **Product development** (architecture-design phase closed; see Project Governance Rule in `AGENTS.md` and doc 40)
- Current sprint: **L1A** Club Intelligence Evidence complete
- Last completed delivery: Sprint **L1A** (`docs/sprints/L1/L1A_CLUB_INTELLIGENCE_EVIDENCE_COMPLETION_REPORT.md`); prior DA, P0, A1.5, A1, R1, I2B, I2A, I1B, I1A, F1.3B, F1.3A, F1.2b, F1.2a, F1.1E
- Demo: recorded cassette `football:100001` includes full xG windows + Match Context + Club Intelligence; odds cassette `match-example` includes O/U + optional market depth; Evidence catalog: `docs/50_EVIDENCE_CATALOG.md`; evaluation demo population + Evaluation History in `@fas/statistics`
- Next authorized work: **L1B Club Intelligence Features / Rule / Confidence / Projection**; parallel **A2 Calibration** (doc 40 trust track)
...
- `@fas/statistics` (pinned `population_demo_v1` frequency-ratio candidate by default; identity still selectable; no match-run training; A1 evaluation + A1.5 Evaluation History repository port / in-memory store)
...
- Workspace separates **Prediction** / **Actual Result** (`MATCH_RESULT`) / **Evaluation** / **Evaluation History** (A1 + A1.5; History is append-only and read-only; never mutates Projection)
