...
- Last updated: 2026-07-30 (P2F Projection V2 Multi-Script Engine — per-script projection + merge in `@fas/analysis` + web workspace)
...
- Current sprint: **P2F** Projection V2 Multi-Script Engine complete (production code). `computeMultiScriptProjection` runs one Poisson projection per activated Match Script; `mergeProbabilityMatrices` (`convex_cell_merge_v1`) produces the final pre-calibration distribution using Match Script weights only; framework exposes per-script λ, 1X2, scorelines, goal range, and merge contributions; workspace shows **激活剧本 → 分剧本预测 → 合并最终预测**. Prior **P2E** Match Script Generator + **P2D** Football State feed this path. **Governance note:** P2A–P2F not yet listed in doc 40.
- Last completed delivery: Sprint **P2F** (`docs/sprints/P2F/P2F_PROJECTION_V2_MULTI_SCRIPT_ENGINE_COMPLETION_REPORT.md`); prior P2E Match Script Generator, P2D, P2E.5, P2E Feature-Enriched Lambda, P2C, P2B, P2A, M1A, O1, V1A, A2, P1B, P1A, L1B, L1A, DA, P0, A1.5, A1, R1, I2B, I2A, I1B, I1A, F1.3B, F1.3A, F1.2b, F1.2a, F1.1E
