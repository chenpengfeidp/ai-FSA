...
- Last updated: 2026-07-30 (P2D Projection V2 Foundation — production code in `@fas/analysis`)
...
- Current sprint: **P2D** Projection V2 Foundation complete (production code). Introduces Projection V2 pipeline in `@fas/analysis` — `ProjectionParameterArtifact` → identity `FootballState` → baseline `MatchScript` → `ProbabilityMatrix` → `ProjectionResult` — with **identical sealed prediction outputs to V1** via delegation to `computeDeterministicMatchProjection`. Default pin remains `"v1"`; V2 selectable via `projectionPolicyPin`. Exposes `projectionFramework` metadata only (report/API; no UI). **Governance note:** P2A/P2B/P2C/P2D not yet listed in doc 40.
- Last completed delivery: Sprint **P2D** (`docs/sprints/P2D/P2D_PROJECTION_V2_FOUNDATION_COMPLETION_REPORT.md`); prior P2C, P2B, P2A, M1A, O1, V1A, A2, P1B, P1A, L1B, L1A, DA, P0, A1.5, A1, R1, I2B, I2A, I1B, I1A, F1.3B, F1.3A, F1.2b, F1.2a, F1.1E
