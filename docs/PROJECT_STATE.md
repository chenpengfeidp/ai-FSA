...
- Last updated: 2026-07-30 (P2E Feature-Enriched Lambda — production code in `@fas/analysis`)
...
- Current sprint: **P2E** Feature-Enriched Lambda complete (production code). `LambdaBuilderV2` in `@fas/analysis` computes λ_home/λ_away from six governed Feature groups (attack, defence, xG, club strength, player availability, match context) via `ProjectionParameterArtifact.lambda`. V2 projection path removes Rule softmax (Rules explainability-only for probabilities). Default pin remains `"v1"`. **Governance note:** P2A–P2E not yet listed in doc 40.
- Last completed delivery: Sprint **P2E** (`docs/sprints/P2E/P2E_FEATURE_ENRICHED_LAMBDA_COMPLETION_REPORT.md`); prior P2D, P2C, P2B, P2A, M1A, O1, V1A, A2, P1B, P1A, L1B, L1A, DA, P0, A1.5, A1, R1, I2B, I2A, I1B, I1A, F1.3B, F1.3A, F1.2b, F1.2a, F1.1E
