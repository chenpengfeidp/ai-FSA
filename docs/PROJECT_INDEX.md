...
7. `docs/protocols/FOOTBALL_INTELLIGENCE_ANALYSIS_PROTOCOL.md`（Agent PRE_MATCH 分析执行与治理；不覆盖 owning contracts）
8. Sprint plans / reports（须引用 doc 40；历史证据不覆盖契约）
9. 现有实现
10. 评论、示例、agent 假设

Agent 规则：`AGENTS.md`（含 Project Governance Rule）→ `PROJECT_STATE.md` → 产品工作再读 doc 40 → 比赛分析再读 canonical protocol。
...
| `docs/protocols/FOOTBALL_INTELLIGENCE_ANALYSIS_PROTOCOL.md` | 唯一 canonical、Provider-agnostic、非 Architecture 的 Agent PRE_MATCH 比赛分析协议；拥有 Agent 执行顺序、研究/覆盖/失败呈现、integrity audit 与输出形状，不拥有 API、Evidence schema 或模型数学。 |
...
00_PROJECT_BIBLE
    ...
    ├── protocols/FOOTBALL_INTELLIGENCE_ANALYSIS_PROTOCOL.md
    │     (canonical Agent PRE_MATCH protocol; non-Architecture)
...
| `@fas/statistics` | 校准 artifact（identity / population_demo_v1）+ A1.5 Evaluation History + A2 Prediction Calibration report + V1A Football Intelligence Validation report + O1 Football Intelligence Contribution report（仅度量，只读 History；从不改写已封存 Prediction）+ authentic PRE_MATCH seal capture（Class A 仅在 postgres 持久化成功后） |
...
31. **P2K-G2-A Validation Dataset Diversity Expansion** — 已完成（`docs/sprints/P2K/P2K_G2_A_VALIDATION_DATA_EXPANSION_COMPLETION_REPORT.md`；`match-p2kg-expansion-v2-*` ×30；28 profiles）  
32. **P2K-E Validation Expansion V2 Sealed Cohort** — 已完成（`docs/sprints/P2K/P2K_E_VALIDATION_EXPANSION_V2_SEALED_COHORT_COMPLETION_REPORT.md`；`p2k.e.validation.expansion.v2.analyzematch.v1` SEALED；30 members；digest `03b52d71…`）  
...
3. `AGENTS.md`（禁区与权威序）  
4. 执行比赛分析时读 `docs/protocols/FOOTBALL_INTELLIGENCE_ANALYSIS_PROTOCOL.md`（唯一 canonical、Provider-agnostic、非 Architecture；未落地 runtime gate 会明确 fail closed）
