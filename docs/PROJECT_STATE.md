...
- Last updated: 2026-07-30 (P2G Unified Probability Matrix — single merged matrix drives all V2 predictions in `@fas/analysis` + web workspace)
...
- Current sprint: **P2G** Unified Probability Matrix complete (production code). Per-script Poisson matrices merge into one unified matrix (`convex_cell_merge_v1`); `deriveMatrixPredictions` (`unifiedMatrix.v1`) derives Winner, scorelines, goal range, BTTS, and Over/Under from merged cells only; framework exposes `unifiedMatrix` summary with derivation notes; workspace shows **分剧本矩阵 → 统一矩阵摘要 → 矩阵衍生预测**. Prior **P2F** Multi-Script Engine + **P2E** Match Script Generator + **P2D** Football State feed this path. **Governance note:** P2A–P2G not yet listed in doc 40.
- Last completed delivery: Sprint **P2G** (`docs/sprints/P2G/P2G_UNIFIED_PROBABILITY_MATRIX_COMPLETION_REPORT.md`); prior P2F, P2E, P2D, P2E.5, P2E Feature-Enriched Lambda, P2C, P2B, P2A, M1A, O1, V1A, A2, P1B, P1A, L1B, L1A, DA, P0, A1.5, A1, R1, I2B, I2A, I1B, I1A, F1.3B, F1.3A, F1.2b, F1.2a, F1.1E
